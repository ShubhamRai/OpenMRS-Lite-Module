        ...
        if (FormEntryContext.Mode.VIEW == context.getMode()) {
            ...
            if (existingDiagnoses != null) {
                ...
                for (Diagnosis diagnosis : existingDiagnoses) {
                    ...
                    sb.append(uiUtils.message("OpenmrsLite.patientDashBoard.diagnosisQuestion." + diagnosis.getOrder()));
                    sb.append("</small><span>");
                    // answer (e.g. "(Confirmed) Malaria [code]")
                    sb.append("(" + uiUtils.message("OpenmrsLite.Diagnosis.Certainty." + diagnosis.getCertainty()) + ") ");
            ...
            try {
                ...
                try {
                    ...
                    output.append(uiUtils.includeFragment("OpenmrsLite", "diagnosis/encounterDiagnoses", fragmentConfig));
    ...
    public Collection<FormSubmissionError> validateSubmission(FormEntryContext context, HttpServletRequest request) {
        ...
        try {
            ...
            if (diagnoses.size() > 0) {
                ...
                if (!foundPrimary) {
                    return Collections.singleton(new FormSubmissionError(hiddenDiagnoses, uiUtils.message("OpenmrsLite.encounterDiagnoses.error.primaryRequired")));
                }
            }
        } catch (IOException e) {
