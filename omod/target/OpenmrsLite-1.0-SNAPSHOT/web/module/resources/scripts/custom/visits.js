...
visit.createQuickVisitCreationDialog = function(patientId) {
    visit.quickVisitCreationDialog = emr.setupConfirmationDialog({
        ...
        actions: {
            confirm: function() {
                emr.getFragmentActionWithCallback('OpenmrsLite', 'visit/quickVisit', 'create',
...
visit.createEndVisitDialog = function() {
    visit.endVisitDialog = emr.setupConfirmationDialog({
        ...
        actions: {
            confirm: function() {
                emr.getFragmentActionWithCallback('OpenmrsLite', 'visit/visitDetails', 'endVisit'
...
visit.createRetrospectiveVisitDialog = function(patientId) {
    visit.retrospectiveVisitCreationDialog = emr.setupConfirmationDialog({
        ...
        actions: {
            confirm: function() {
                emr.getFragmentActionWithCallback('OpenmrsLite', 'visit/retrospectiveVisit', 'create',
                            ...
                            data.forEach(function (v) {
                                ...
                                listItem.click(function() {
									if (jq('.viewVisitDetails').length) {
										...
										window.location = visit.contextPath + "/OpenmrsLite/patientdashboard/patientDashboard.page?patientId=" + patientId + "&visitId=" + v.id + "#visits";
									}
                                });
...
function showEditVisitDateDialog(visitId) {
    if (!editVisitDialogs[visitId]) {
        editVisitDialogs[visitId] = emr.setupConfirmationDialog({
            ...
            actions: {
                confirm: function() {
                    var url = emr.fragmentActionLink("OpenmrsLite", "visit/visitDates", "setDuration");
