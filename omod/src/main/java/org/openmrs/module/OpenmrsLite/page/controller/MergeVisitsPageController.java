...
public class MergeVisitsPageController {
    public Object controller(@RequestParam("patientId") Patient patient,
        ...
        if (patient.isVoided() || patient.isPersonVoided()) {
            return new Redirect("OpenmrsLite", "patientdashboard/deletedPatient", "patientId=" + patient.getId());
        }
    ...
    public String post(@RequestParam("patientId") Patient patient,
        ...
        if (patient.isVoided() || patient.isPersonVoided()) {
            return "redirect:" + ui.pageLink("OpenmrsLite", "patientdashboard/deletedPatient",
        ...
        if (mergeVisits!=null && mergeVisits.size() > 0 ){
            ...
            if (mergedVisit != null){
                request.getSession().setAttribute(AppUiConstants.SESSION_ATTRIBUTE_INFO_MESSAGE, ui.message("OpenmrsLite.task.mergeVisits.success"));
                request.getSession().setAttribute(AppUiConstants.SESSION_ATTRIBUTE_TOAST_MESSAGE, "true");
            }else{
                request.getSession().setAttribute("emr.errorMessage", ui.message("OpenmrsLite.task.mergeVisits.error"));
                request.getSession().setAttribute(AppUiConstants.SESSION_ATTRIBUTE_TOAST_MESSAGE, "true");
            }
        }

		return "redirect:" + ui.pageLink("OpenmrsLite", "mergeVisits", SimpleObject.create("patientId", patient.getId(), "returnUrl", returnUrl));
    }
}
