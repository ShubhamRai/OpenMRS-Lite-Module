        ...
        <h1 class="name">
            ...
            <span class="gender-age">
                ...
                <% if(!config.hideEditDemographicsButton) { %>
                    <span>
                        <small><a href="/${contextPath}/OpenmrsLite/editPatientDemographics.page?patientId=${patient.patient.id}&returnUrl=${ui.urlEncode(ui.thisUrl())}">${ui.message("general.edit")}</a></small>
                    </span>
                <% } %>
                <% if (config.isNewPatientHeaderEnabled) { %>
