<%
    ...
    ui.includeJavascript("OpenmrsLite", "diagnoses/diagnoses.js")
    ui.includeJavascript("OpenmrsLite", "diagnoses/diagnoses-angular.js")
    ui.includeCss("OpenmrsLite", "diagnoses/encounterDiagnoses.css")
%>
...
<script type="text/template" id="autocomplete-render-item">
    <span class="code">
        ...
        ${ui.message("OpenmrsLite.consult.codedButNoCode")}
        {{ } else { }}
        ${ui.message("OpenmrsLite.consult.nonCoded")}
        {{ } }}
    </span>
    ...
    <span class="preferred-name">
        <small>${ui.message("OpenmrsLite.consult.synonymFor")}</small>
...
<div id="encounter-diagnoses-app">
        ...
        <div class="diagnosis" data-ng-class="{primary: d.primary}">
            ...
            <div class="actions">
                <label>
                    ...
                    ${ui.message("OpenmrsLite.Diagnosis.Order.PRIMARY")}
                </label>
                <label>
                    <input type="checkbox" data-ng-model="d.confirmed"/>
                    ${ui.message("OpenmrsLite.Diagnosis.Certainty.CONFIRMED")}
                </label>
            </div>
        </div>
    ...
    <div data-ng-controller="DiagnosesController">
        ...
        <div id="diagnosis-search-container">
            <label for="diagnosis-search">${ ui.message("OpenmrsLite.consult.addDiagnosis") }</label>
            <input id="diagnosis-search" type="text" placeholder="${ ui.message("OpenmrsLite.consult.addDiagnosis.placeholder") }" autocomplete itemFormatter="autocomplete-render-item"/>

            <% if(jsForPrior.size > 0) { %>
                <button type="button" ng-click="addPriorDiagnoses()">${ ui.message("OpenmrsLite.consult.priorDiagnoses.add") }</button>
            <% } %>
        </div>

        <div id="display-encounter-diagnoses-container">
            <h3>${ui.message("OpenmrsLite.consult.primaryDiagnosis")}</h3>

            <div data-ng-show="encounterDiagnoses.primaryDiagnoses().length == 0">
                ${ui.message("OpenmrsLite.consult.primaryDiagnosis.notChosen")}
            </div>
            <ul>
            ...
            <h3>${ui.message("OpenmrsLite.consult.secondaryDiagnoses")}</h3>

            <div data-ng-show="encounterDiagnoses.secondaryDiagnoses().length == 0">
                ${ui.message("OpenmrsLite.consult.secondaryDiagnoses.notChosen")}
            </div>
            <ul>
