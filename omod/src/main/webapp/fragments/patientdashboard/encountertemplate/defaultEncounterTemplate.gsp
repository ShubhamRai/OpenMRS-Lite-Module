<%	
	ui.includeJavascript("OpenmrsLite", "fragments/patientdashboard/encountertemplate/defaultEncounterTemplate.js")
%>
...
<li>
	    ...
	    <li>
	        <div class="details-action">
	            <a class="view-details collapsed" href='javascript:void(0);' data-encounter-id="{{- encounter.encounterId }}" data-encounter-form="{{- encounter.form != null}}" data-display-with-html-form="{{- config.displayWithHtmlForm }}" data-target="#encounter-summary{{- encounter.encounterId }}" data-toggle="collapse" data-target="#encounter-summary{{- encounter.encounterId }}">
	                <span class="show-details">${ ui.message("OpenmrsLite.patientDashBoard.showDetails")}</span>
	                <span class="hide-details">${ ui.message("OpenmrsLite.patientDashBoard.hideDetails")}</span>
	...
	<span>
        {{ if ( config.editable && encounter.canEdit) { }}
            <i class="editEncounter delete-item icon-pencil" data-patient-id="{{- patient.id }}" data-encounter-id="{{- encounter.encounterId }}" {{ if (config.editUrl) { }} data-edit-url="{{- config.editUrl }}" {{ } }} title="${ ui.message("OpenmrsLite.edit") }"></i>
        {{ } }}
        {{ if ( encounter.canDelete ) { }}
	       <i class="deleteEncounterId delete-item icon-remove" data-visit-id="{{- encounter.visitId }}" data-encounter-id="{{- encounter.encounterId }}" title="${ ui.message("OpenmrsLite.delete") }"></i>
        {{  } }}
	</span>
...
<script type="text/template" id="defaultEncounterDetailsTemplate">
    ...
    {{ _.each(orders, function(order) { }}
        <p>
            <small>${ ui.message("OpenmrsLite.patientDashBoard.accessionNumber")}</small><span>{{- order.accessionNumber }}</span>
        </p>
        <p>
            <small>${ ui.message("OpenmrsLite.patientDashBoard.order")}</small><span>{{- order.concept }}</span>
        </p>
    {{ }); }}
</script>
