...
<script type="text/javascript">
    var breadcrumbs = [
        ...
        { label: "${ ui.message("OpenmrsLite.task.mergeVisits.label")}"}
    ];
...
<h3>${ ui.message("OpenmrsLite.task.mergeVisits.label") }</h3>
${ ui.message("OpenmrsLite.task.mergeVisits.instructions") }

<form method="post" action="${ ui.pageLink("OpenmrsLite", "mergeVisits") }">
    <table id="active-visits" width="100%" border="1" cellspacing="0" cellpadding="2">
        <thead>
        <tr>
            <th>${ ui.message("OpenmrsLite.merge") }</th>
            <th>${ ui.message("OpenmrsLite.startDate.label") }</th>
            <th>${ ui.message("OpenmrsLite.stopDate.label") }</th>
            <th>${ ui.message("OpenmrsLite.patientDashBoard.encounters") }</th>
        </tr>
        </thead>
        <tbody>
    ...
    <div>
        <input type="button" class="cancel" value="${ ui.message("OpenmrsLite.cancel") }" onclick="javascript:window.location='${ returnUrl }'" />
        <input type="submit" id="mergeVisitsBtn" class="confirm" value="${ ui.message("OpenmrsLite.task.mergeVisits.mergeSelectedVisits") }" />
    </div>
</form>
